...
let carState = {
  ...
  can_ready: false,
  lock_learned: false,
  unlock_learned: false,
  engine_learned: false,
  climate_learned: false,
};
...
app.post('/api/climate', (req, res) => {
  ...
  if (temp !== undefined && (temp < 16 || temp > 30)) {
    return res.status(400).json({
      status: 'error',
      message: 'Temperatura invalida (16-30)',
    });
  }
  ...
  if (sent) {
    carState.ac_on = payload.on;
    carState.ac_temp = payload.temp;
    carState.ac_fan = payload.fan;
  }
  ...
  apiResponse(
    ...
    sent,
    sent
      ? (payload.on ? 'Climatizare pornita' : 'Climatizare oprita')
...
app.post('/api/learn', (req, res) => {
  ...
  const hints = {
    lock: 'Apasa LOCK pe cheie in 5 secunde!',
    unlock: 'Apasa UNLOCK pe cheie in 5 secunde!',
    engine_start: 'Porneste motorul in 5 secunde!',
    engine_stop: 'Opreste motorul in 5 secunde!',
    climate: 'Regleaza clima in 5 secunde!',
  };

  res.json({
    status: 'ok',
    message: hints[action] || 'Fa actiunea in 5 secunde!',
    delivered: sent,
  });
});
...
app.get('/', (req, res) => {
  res.json({
    service: 'Mercedes W213 Cloud Bridge',
    status: 'running',
    esp32: carState.connected ? 'online' : 'offline',
  });
});
